"""
Optimized analytical training with torch.compile and BFloat16 support.

This module implements high-performance training with:
- torch.compile for sweep-line algorithm
- BFloat16 mixed precision for 2x throughput on RTX 5090
- Float packing for fast GPU sorting
- Exact analytical integral loss computation
"""
...
def sort_and_drop_duplicates(t_vals, batch_ids, values=None, eps=1e-6):
    """
    Sorts flattened knots by (Batch_ID, T) and removes duplicates within the same batch.
    
    Uses float packing trick for fast GPU sorting (no lexsort).
    ...
    # Float packing trick: batch_id + t_vals * 0.99
    # This ensures t=1 doesn't spill to next batch
    sort_key = batch_ids.float() + (t_vals * 0.99)
...
def reset_predictions_tensor_mode(mlp: nn.Module, p0: torch.Tensor, p1: torch.Tensor):
    ...
    Designed to be compatible with torch.compile.
    ...
    for i, layer in enumerate(layers):
        ...
        if len(batch_ids) > 1:
            ...
            if mask_seg.any():
                ...
                # Solve: Z_L + alpha * (Z_R - Z_L) = 0
                denom = z_R - z_L  # (N_segs, D_out)
                # Avoid div/0
                safe_denom = torch.where(
                    torch.abs(denom) < 1e-9,
                    torch.tensor(1e-9, device=device, dtype=denom.dtype),
                    denom
                )
...
def batch_exact_integral_loss(pred_t, pred_v, gt_t, gt_v):
    ...
    This implements analytical integration for piecewise-linear functions.
...
def collate_splines(splines: List[Spline], device, use_bfloat16: bool = False) -> Dict[str, torch.Tensor]:
    """
    ...
    # Start/End points (keep in FP32 for numerical stability)
    p0 = torch.stack([s.start_point for s in splines]).to(device=device, dtype=torch.float32)
    p1 = torch.stack([s.end_point for s in splines]).to(device=device, dtype=torch.float32)
    
    # GT Data (convert to target precision)
    gt_t_list = [s.gt_knots.to(device=device, dtype=dtype) for s in splines]
    gt_v_list = [s.gt_values.to(device=device, dtype=dtype) for s in splines]
...
def train_model_optimized(
    ...
    """
    High-performance training loop with torch.compile and mixed precision.
    ...
    Args:
        ...
        use_compile: Whether to use torch.compile (enabled by default)
        precision: Precision mode ('fp32', 'bfloat16', 'fp16')
    ...
    use_mixed_precision = use_bfloat16 or use_fp16
    
    if use_bfloat16:
        mlp.to(dtype=torch.bfloat16)
        dtype = torch.bfloat16
    elif use_fp16:
        mlp.to(dtype=torch.float16)
        dtype = torch.float16
    else:
        dtype = torch.float32
    ...
    if torch.cuda.is_available():
        ...
        print(f"CUDA memory:         {torch.cuda.memory_allocated(0) / 1e9:.2f} GB allocated")
    print("="*60 + "\n")
    
    # Compile the forward pass if requested
    if use_compile:
        print("Compiling model with torch.compile (mode='reduce-overhead')...")
        forward_fn = torch.compile(reset_predictions_tensor_mode, mode="reduce-overhead")
        print("✓ Compilation complete\n")
    else:
        forward_fn = reset_predictions_tensor_mode
    
    optimizer = optim.Adam(mlp.parameters(), lr=lr)
    
    # Pre-process data once
    print("Collating spline data...")
    data = collate_splines(splines, device, use_bfloat16=use_mixed_precision)
    N = len(splines)
    ...
    for epoch in pbar:
        ...
        for i in range(0, N, batch_size):
            ...
            if use_mixed_precision:
                with torch.autocast(device_type='cuda', dtype=dtype):
                    ...
                    pred_t, pred_v = forward_fn(mlp, p0, p1)
        ...
        if avg_loss < best_loss:
            ...
            if save_path:
                ...
                }, save_path / 'best_model_optimized.pt')
...
def update_spline_predictions(mlp: ReluMLP, splines: List[Spline], use_compile: bool = False):
    """
    ...
    Args:
        ...
        use_compile: Whether to use compiled version
    """
    device = splines[0].start_point.device
    mlp = mlp.to(device)
    
    forward_fn = torch.compile(reset_predictions_tensor_mode, mode="reduce-overhead") if use_compile else reset_predictions_tensor_mode
    
    # Process in batches to avoid memory issues
    batch_size = 128
    
    for i in tqdm(range(0, len(splines), batch_size), desc="Updating predictions"):
