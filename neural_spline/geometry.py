"""
Robust constrained recursive PCA for geometric decomposition.

This module implements stable recursive PCA with:
- Mass-based eigenvector orientation for consistency
- Degeneracy handling for near-symmetric regions
- Slab method clipping with convex polytope tracking
...
def orient_eigenvector_by_mass(points: torch.Tensor, center: torch.Tensor, 
    ...
    """
    # Project centered points onto eigenvector
    centered = points - center
    projections = centered @ eigenvector  # (N,)
    
    # Count positive projections
    n_positive = (projections > 0).sum().item()
    n_total = len(points)
    
    # If less than half are positive, flip the vector
...
def constrained_recursive_pca(
    ...
    # Apply mass-based orientation to all axes
    oriented_axes = []
    for i in range(n_dims):
        oriented_axis = orient_eigenvector_by_mass(points, center, axes[i])
        oriented_axes.append(oriented_axis)
    axes = torch.stack(oriented_axes, dim=0)
    
    # Resolve degeneracy if present
    ...
    for i in range(n_dims):
        ...
        component = PCAComponent(
            ...
            label=label,
            polytope=polytope,
            parent_axis=axes[i] if i == 0 else None  # Store first axis for children
        )
    ...
    for subdiv_idx in range(n_subdivisions):
        ...
        for dim in range(n_dims):
            ...
            # Mask for this dimension
            if sign > 0:
                dim_mask = (projections[:, dim] >= 0)
                subdiv_label += '+'
                # Add constraint: axis_dim · (x - center) >= 0
                # Rewrite as: -axis_dim · x <= -axis_dim · center
                normal = -axes[dim]
                offset = -torch.dot(axes[dim], center).item()
            else:
                dim_mask = (projections[:, dim] <= 0)
                subdiv_label += '-'
                # Add constraint: axis_dim · (x - center) <= 0
